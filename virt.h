...
double Rate_1_to_2( double o, double k, // K = (\omega,k)
  ...
  int integrand(unsigned dim,  const double *x, void *data_,
    ...
    pA*(  - (.0+n(sA,eA-uA))*calF( +.5*(SQR(mB)+SQR(pA)+SQR(pD)-SQR(eD-eA)), _ab)
          + (.0+n(sA,eA+uA))*calF( -.5*(SQR(mB)+SQR(pA)+SQR(pD)-SQR(eD+eA)), _ab)  );

    _ab[0] = creal(pB*pD*cos_kpD*_Z_);
    _ab[1] = creal(pB*pD*csqrt(1.-SQR(cos_kpD))*csqrt(1.-SQR(_Z_)));
    double complex tempB = 
    pB*(  - (.0+n(sB,eB-uB))*calF( +.5*(SQR(mA)+SQR(pB)+SQR(pD)-SQR(eD-eB)), _ab)
          + (.0+n(sB,eB+uB))*calF( -.5*(SQR(mA)+SQR(pB)+SQR(pD)-SQR(eD+eB)), _ab)  );
